...
describe('PatientsService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw BadRequestException on database error', async () => {
      ...
      await expect(service.create(createPatientDto)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('findAll', () => {
    ...
    it('should return all patients without query', async () => {
      ...
      expect(mockQueryBuilder.orderBy).toHaveBeenCalledWith('patient.name', 'ASC');
    ...
    it('should filter patients by name', async () => {
      ...
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('patient.name LIKE :name', {
        name: '%Doe%',
      });
      expect(result).toEqual([mockPatients[0]]);
    });
  ...
  describe('findByMedicalRecordNumber', () => {
    ...
    it('should throw NotFoundException if patient not found', async () => {
      ...
      await expect(service.findByMedicalRecordNumber('MRN-999')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('update', () => {
    ...
    it('should throw ConflictException if medical record number already exists', async () => {
      ...
      await expect(service.update(1, updateWithMRN)).rejects.toThrow(ConflictException);
    });

    it('should throw NotFoundException if patient not found', async () => {
      mockRepository.findOne.mockResolvedValue(null);

      await expect(service.update(999, updatePatientDto)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('validateMedicalRecordNumber', () => {
    ...
    it('should exclude specific patient ID when validating', async () => {
      ...
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('patient.id != :excludeId', {
        excludeId: 1,
      });
      expect(result).toBe(true);
    });
